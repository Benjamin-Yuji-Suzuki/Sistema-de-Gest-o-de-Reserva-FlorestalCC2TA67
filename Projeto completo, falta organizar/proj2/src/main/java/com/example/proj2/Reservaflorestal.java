...
public class Reservaflorestal {
   ...
   try (Writer fileWriter = new OutputStreamWriter(new FileOutputStream(caminhoArquivo), StandardCharsets.UTF_8);
    ...
    // Escreve o BOM no início do arquivo para que o Excel reconheça UTF-8
    fileWriter.write('\uFEFF');
    ...
    for (Avistamento avistamento : avistamentos) {
        ...
        if (area != null && especie != null) {
            String[] linha = {
                ...
                especie.getTipo(),
                especie.getCondicao(),
                avistamento.getData().toString() // Certifique-se que getData() retorne uma string adequada
            };
    ...
    try (Connection conn = DatabaseConnection.getConnection()) {
        ...
        // Lê o arquivo CSV com codificação UTF-8 e separador ponto e vírgula
        try (CSVReader reader = new CSVReaderBuilder(
                new InputStreamReader(new FileInputStream(caminhoArquivo), StandardCharsets.UTF_8))
                .withCSVParser(new CSVParserBuilder().withSeparator(';').build()) // Define o separador como ';'
                .build()) {
            ...
            while ((linha = reader.readNext()) != null) {
                ...
                try {
                    ...
                    String data = linha[7].trim(); // Mantemos data como String
                    ...
                    // Registra o avistamento (data agora é uma String, como antes)
